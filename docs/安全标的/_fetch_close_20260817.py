# -*- coding: utf-8 -*-
"""2026-08-17 收盘数据拉取：A股指数 + 观察池55只 + 持仓8只（腾讯行情接口）"""
...
# 持仓（含8/17已减仓后洛钼8手）
HOLD = {
    "sh601766": {"name": "中国中车", "hands": 4, "cost": 5.728},
    ...
    "sh600999": {"name": "招商证券", "hands": 5, "cost": 19.840},
...
def main():
    ...
    for c, h in HOLD.items():
        if c in result:
            ...
            total_val += val
            hold_info.append((h["name"], r["price"], r["pct"], val, pnl, pnl_pct))
